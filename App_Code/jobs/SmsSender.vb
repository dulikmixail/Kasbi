...
Namespace Jobs
    Public Class SmsSender
        ...
        Public Function FindLongTimeSkno() As Dictionary(Of Integer, List(Of String))
            ...
            Using ds = New DataSet()
                Using con = ServiceDbConnector.GetSharedConnection()
                    Using adapt As SqlClient.SqlDataAdapter = con.GetDataAdapter("get_good_with_long_time_skno", True)
                        adapt.Fill(ds)
                    End Using
                End Using
                For Each dr As DataRow In ds.Tables(0).Rows
                    ...
                    If _
                        sknoReceivedUpdateDate.AddDays(DaysBeforeSendingSknoSms) < Now() And
                        Not IsDBNull(dr("tel_notice"))
                        longTimeSkno.Add(Convert.ToInt32(dr("good_sys_id")),
                                         New List(Of String) _
                                            From {dr("tel_notice"), Trim(dr("num_cashregister").ToString())})
                    End If
                Next
            End Using
        ...
        Public Function FindLongTimeRepair() As Dictionary(Of Integer, List(Of String))
            ...
            Using ds = New DataSet()
                Using cmd = New SqlClient.SqlCommand("get_repair_with_long_time")
